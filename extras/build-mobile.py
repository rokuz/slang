#!/usr/bin/env python3
"""
Build Slang for iOS and Android, then publish as a GitHub release on a fork.
...
Requirements:
    ...
    - Android NDK with ANDROID_NDK set (for Android builds)
...
The script:
    ...
    4. Cross-compiles for iOS device (arm64), iOS Simulator (arm64), and Android (arm64-v8a + x86_64)
...
def build_android(tag: str, generators_bin: Path, staging_dir: Path) -> list[Path]:
    ...
    if not ndk_home:
        print("ERROR: ANDROID_NDK_HOME (or ANDROID_NDK) is not set. Skipping Android builds.")
        print("       Set it to your NDK installation path, e.g.:")
        print("       export ANDROID_NDK_HOME=$HOME/Library/Android/sdk/ndk/<version>")
        return []

    # CMakePresets.json expands $env{ANDROID_NDK_HOME}; ensure it's set even
    # when only the legacy ANDROID_NDK variable was provided.
    cmake_env = {**os.environ, "ANDROID_NDK_HOME": ndk_home}
    ...
    for abi, preset, build_dir_name in archs:
        ...
        run(
            ["cmake", "--preset", preset, "--fresh",
             f"-DSLANG_GENERATORS_PATH={generators_bin}"],
            quiet=True, cwd=ROOT_DIR, env=cmake_env,
        )
...
- `lib/` — libraries (iOS: static `.a`, Android: shared `.so`)
- `share/doc/slang/` — documentation

### Usage
**iOS / iOS Simulator**: Link against the static libraries (`.a`).
...
def main():
    ...
    _ALL_PLATFORMS = ("ios", "ios-simulator", "android")
